        ...
        void Render(uint bounces){
            ...
            for (int y = 0; y < h; ++y){
                for (int x = 0; x < w; ++x){
                    ...
                    data[(h-1-y)*w+x].x = (uint)(std::clamp(screen_rays[x*h+y].color.x, 0.0f, 1.0f)*255);
                    data[(h-1-y)*w+x].y = (uint)(std::clamp(screen_rays[x*h+y].color.y, 0.0f, 1.0f)*255);
                    data[(h-1-y)*w+x].z = (uint)(std::clamp(screen_rays[x*h+y].color.z, 0.0f, 1.0f)*255);
                }
            }
